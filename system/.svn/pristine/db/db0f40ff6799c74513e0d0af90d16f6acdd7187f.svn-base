...
<head>
	...
	<script src="${pageContext.request.contextPath}/resources/manager/current/js/modemList.js"></script>
	</c:if>
								...
								<div class="card shadow mb-4">
									<div class="card-body">
										<div style="width: 100%; display: inline-block;">
											<form id="searchForm" name="searchForm" method="post" action="<c:url value='../current/modemList'/>">
									...
									<div class="row">
										<div class="col-sm-12">
											<table class="table tablesorter" id="dataTable" style="text-align: center;">
												...
												<tbody>
													<c:forEach items="${LIST_MODEM}" var="LIST_MODEM">
														<tr>
															<td width="10%">${LIST_MODEM.dcu_id}</td>
															<td width="10%">${LIST_MODEM.mac_modem}</td>
															<td width="15%">${LIST_MODEM.site_name}</td>
															<td width="8%">${LIST_MODEM.dong_name}</td>
															<td width="8%">${LIST_MODEM.ho_name}</td>
															<td width="10%">${LIST_MODEM.fw_version}</td>
															<td width="7%">${LIST_MODEM.count_meter}</td>
															<td width="7%">${LIST_MODEM.worker_name}</td>
															<td width="10%"></td>
															<td width="15%">${LIST_MODEM.time_last_comm}</td>
														</tr>
													</c:forEach>
												</tbody>
											</table>
											...
											<div class="dataTables_paginate paging_simple_numbers"
												...
												<c:if test="${PAGEUTIL.startPage eq 1}">
													...
													<li class="paginate_button page-item previous" id="">
														<a href="../current/modemList?nowPage=${PAGEUTIL.startPage - 1 }&SeqSite=${SEQSITE}&SeqDong=${SEQ_APT_DONG}&SeqDcu=${SEQDCU}" aria-controls="dataTable" data-dt-idx="0" tabindex="0" class="page-link">
															<
														</a>
													</li>			
													</c:if>
													...
													<li class="paginate_button page-item" id="">
														<a href="../current/modemList?nowPage=1&SeqSite=${SEQSITE}&SeqDong=${SEQ_APT_DONG}&SeqDcu=${SEQDCU}" aria-controls="dataTable" data-dt-idx="0" tabindex="0" class="page-link">
															1
														</a>
													</li>			
													</c:if>
													...
													<li class="paginate_button page-item" id="">
														<a href="../current/modemList?nowPage=${page}&SeqSite=${SEQSITE}&SeqDong=${SEQ_APT_DONG}&SeqDcu=${SEQDCU}" aria-controls="dataTable" data-dt-idx="0" tabindex="0" class="page-link">
															${page}
														</a>
													</li>			
													</c:forEach>
													...
													<li class="paginate_button page-item next" id="dataTable_previous">
														<a href="../current/modemList?nowPage=${PAGEUTIL.endPage+1}&SeqSite=${SEQSITE}&SeqDong=${SEQ_APT_DONG}&SeqDcu=${SEQDCU}" class="page-link">
															>
														</a>
													</li>		
													</c:if>
