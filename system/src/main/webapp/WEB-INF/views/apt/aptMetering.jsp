...
<c:if test="${not empty sessionScope}">
	<script src="${pageContext.request.contextPath}/resources/manager/apt/js/aptMetering.js"></script>
									...
									<div class="card-body">
										<div id="dataTable_wrapper" class="dataTables_wrapper dt-bootstrap4">
											<div style="width: 100%; display: inline-block;" class="col-sm-6-hs">
												<form id="searchForm" name="searchForm" method="post" action="<c:url value='../apt/metering'/>">
													...
													<!-- 기간 선택  -->
													<div style="margin-right: 15px; margin-left: 15px; float: left; color: white;">
														<span style="display: inline-block; height: 21px;"></span>기간 선택
													</div>
													
													<div style="width: 120px; float: left; margin-right: 10px;">
														<input class="custom-date custom-date-sm" id="To" name="To" type="text" 
														style="background: #182241; color: #dfdfdf; border: 1px solid #485998;" value="${TO_YYMMDD}">
													</div>
													
													<input type="hidden" id="to_yymmdd" name="to_yymmdd" value="${TO_YYMMDD}">
													
													<div style="width: 55px; float: left;">
														<select id="to_selecthhmm" name="seq_apt_dong" class="custom-date custom-date-sm" data-live-search="true"
														style="background: #182241; color: #dfdfdf; border: 1px solid #485998;">
														  <c:forEach var="i"  begin="0" end="23">
														     <c:set var="endTm" value="${i>9?i:'0'}${i>9?'':i}" />
														        <option value="${i>9?i:'0'}${i>9?'':i}00" 
														        <c:if test="${TO_HH eq endTm}">selected</c:if>>${i>9?i:'0'}${i>9?'':i}</option>
														    </c:forEach>
														</select> 
													</div>
													
													<input type="hidden" id="to_hhmm" name= "to_hhmm" value="${TO_HHMM}">
													<input type="hidden" id="to_hh" name= "to_hh" value="${TO_HH}">

													<div style="float: left; padding-left: 15px;">
														<input type="button" id="searchBtn" class="btn btn-outline-primary btn-sm" value="조회">
														<input type="button" id="specialCancle" class="btn-info btn-sm" value="실시간 검침값 조회 ➞"
															style="margin-left: 50px;" onclick="location.href='../apt/meteringNew'">
													</div>
												...
												<form name="meterExcelForm" id="meterExcelForm" method="post">
													...
													<input type="hidden" readonly id="SiteName" name="SiteName" value="${SITENAME}">
			                                       	<input type="hidden" readonly value="${TO}" id="To" name="To">
			                                       	<input type="hidden" readonly value="${TOHHMM}" id="Tohhmm" name="Tohhmm">
